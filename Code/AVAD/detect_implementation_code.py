...
def test2(dist_model, avfeature_model, loader, dist_reg_model, avfeature_reg_model, video_path, max_len=50, csv_writer=None):
        ...
        with tqdm(total=len(loader), position=0, leave=False, colour='green', ncols=150) as pbar:
            for nm, aud_vis in enumerate(loader):
                ...
                if csv_writer is not None:
                    ...
                    csv_writer.writerow([filename, prob.item()])
...
def main():
    ...
    sync_model_weight = torch.load('sync_model.pth', map_location=torch.device('cuda:0'))
    ...
    real_or_fake = parts[-1]
    csv_file_path = os.path.join(output_dir, f'{dataset_name}_{real_or_fake}_score.csv')
    ...
    with open(csv_file_path, mode='a', newline='') as file:
        ...
        with tqdm(total=len(video_paths)) as pbar:
            for video_path in video_paths:
                try:
                    test_video = FakeAVceleb([video_path], opts.resize, opts.fps, opts.sample_rate, vid_len=opts.vid_len, phase=0, train=False, number_sample=1, lrs2=False, need_shift=False, lrs3=False, kodf=False, lavdf=False, robustness=False, test=True)
                    loader_test = DataLoader(test_video, batch_size=opts.bs, num_workers=opts.n_workers, shuffle=False)
                    test2(sync_model, avfeature_sync_model, loader_test, dist_reg_model=dist_regressive_model, avfeature_reg_model=avfeature_regressive_model, video_path=video_path, max_len=opts.max_len, csv_writer=csv_writer)
                except Exception as e:
                    print(video_path, e)
                    video_filename = os.path.basename(video_path)
                    csv_writer.writerow([video_filename, "Error"])
                pbar.update(1)
